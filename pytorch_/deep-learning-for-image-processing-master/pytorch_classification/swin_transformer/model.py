...
def drop_path_f(x, drop_prob: float = 0., training: bool = False):
    ...
    output = x.div(keep_prob) * random_tensor #以广播机制使得某些批次全为0，也就是让某些批次失活
    return output#该drop_path_f函数主要是让某些批次失活减少过拟合
...
class BasicLayer(nn.Module):
    ...
    def create_mask(self, x, H, W):
        ...
        mask_windows = mask_windows.view(-1, self.window_size * self.window_size)  # [nW, Mh*Mw]     （窗口数*窗口大小）
...
def swin_large_patch4_window12_384_in22k(num_classes: int = 21841, **kwargs):
    ...
    model = SwinTransformer(in_chans=3,
                            ...
                            **kwargs)
    return model
model=swin_tiny_patch4_window7_224(10)
